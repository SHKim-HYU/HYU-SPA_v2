...
Vector6nd Liedynamics::Vdot_base(int axis)
{
	...
	case _RotX:
		...
		res.head(6) =LieOperator::AdjointMatrix(inverse_SE3(T[0][1]))*res1;   //MR P.90
		break;
	case _RotY:
		res1 << 0, 0, 0, 0, gravity, 0;
		res.head(6) =LieOperator::AdjointMatrix(inverse_SE3(T[0][1]))*res1;
		break;
	case _RotZ:
